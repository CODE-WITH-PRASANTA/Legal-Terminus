...
const GSTProcess = () => {
  return (
    <section className="cir-gst-wrapper">
      <h2 className="gst-heading">
        STEPS FOR CHANGE IN REGISTERED OFFICE ADDRESS IN INDIA
      </h2>
      ...
      <div className="cir-timeline">
        <div className="cir-timeline-line" />

        {steps.map((step, index) => (
          <div
            key={index}
            className={`cir-timeline-item ${
              index % 2 === 0 ? "left" : "right"
            }`}
          >
            <div className="cir-timeline-dot">{index + 1}</div>

            <div className="cir-timeline-card">
              <h4>{step.title}</h4>
              <p>{step.text}</p>
            </div>
          </div>
