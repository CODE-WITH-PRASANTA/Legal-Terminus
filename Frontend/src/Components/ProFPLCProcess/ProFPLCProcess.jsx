...
const GSTProcess = () => {
  return (
    <section className="profplc-process-wrapper">
      <h2 className="profplc-process-heading">
        STEPS FOR PRIVATE LIMITED COMPANY REGISTRATION IN INDIA
      </h2>
      <p className="profplc-process-subheading">
        The broad process of registering a Private Limited company involves
        the following steps:
      </p>

      <div className="profplc-process-timeline">
        <div className="profplc-process-timeline-line" />

        {steps.map((step, index) => (
          <div
            key={index}
            className={`profplc-process-timeline-item ${
              index % 2 === 0 ? "left" : "right"
            }`}
          >
            <div className="profplc-process-timeline-dot">
              {index + 1}
            </div>

            <div className="profplc-process-timeline-card">
              <h4>{step.title}</h4>
              <p>{step.text}</p>
            </div>
          </div>
