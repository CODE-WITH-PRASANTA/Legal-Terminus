...
import "./TrademarktoOppositionElegibility.css";
...
const TradeLicenseElegibility = () => {
  return (
    <section className="tlelx-wrapper">
      <h2 className="tlelx-heading">
        STEPS FOR TRADE LICENSE REGISTRATION IN INDIA
      </h2>
      <p className="tlelx-subheading">
        The broad process of obtaining a Trade License involves the
        following key steps:
      </p>

      <div className="tlelx-timeline">
        <div className="tlelx-timeline-line" />

        {steps.map((step, index) => (
          <div
            key={index}
            className={`tlelx-timeline-item ${
              index % 2 === 0 ? "left" : "right"
            }`}
          >
            <div className="tlelx-timeline-dot">{index + 1}</div>

            <div className="tlelx-timeline-card">
              <h4>{step.title}</h4>
              <p>{step.text}</p>
            </div>
          </div>
