...
import "./PartnershipLLPProcess.css";
...
const PartnershipLLPProcess = () => {
  return (
    <section className="partnership-llp-process-wrapper">
      <h2 className="partnership-llp-process-heading">
        STEPS FOR PRIVATE LIMITED COMPANY REGISTRATION IN INDIA
      </h2>
      <p className="partnership-llp-process-subheading">
        The broad process of registering a Private Limited company involves
        the following steps:
      </p>

      <div className="partnership-llp-process-timeline">
        <div className="partnership-llp-process-timeline-line" />

        {steps.map((step, index) => (
          <div
            key={index}
            className={`partnership-llp-process-timeline-item ${
              index % 2 === 0 ? "left" : "right"
            }`}
          >
            <div className="partnership-llp-process-timeline-dot">
              {index + 1}
            </div>

            <div className="partnership-llp-process-timeline-card">
              <h4>{step.title}</h4>
              <p>{step.text}</p>
            </div>
          </div>
...
export default PartnershipLLPProcess;
