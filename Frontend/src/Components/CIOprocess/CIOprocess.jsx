...
const GSTProcess = () => {
  return (
    <section className="cioo-gst-wrapper">
      <h2 className="gst-heading">
        STEPS FOR CHANGE IN OBJECT IN INDIA
      </h2>
      ...
      <div className="cioo-timeline">
        <div className="cioo-timeline-line" />

        {steps.map((step, index) => (
          <div
            key={index}
            className={`cioo-timeline-item ${
              index % 2 === 0 ? "left" : "right"
            }`}
          >
            <div className="cioo-timeline-dot">{index + 1}</div>

            <div className="cioo-timeline-card">
              <h4>{step.title}</h4>
              <p>{step.text}</p>
            </div>
          </div>
