...
import "./TradeLicenseOurClients.css";
...
const TradeLicenseOurClients = () => {
  const logos = [
    ...
    // include all your other clients (11–119)
  ];
  ...
  const scrollByCard = (dir) => {
    ...
    if (!track) return;
    const scroller = track.parentElement;
    const card = track.querySelector(".tradeclients-card");
  ...
  return (
    <section className="tradeclients-root">
      <div className="tradeclients-container">
        <h2 className="tradeclients-title">OUR CLIENTS</h2>

        <div className="tradeclients-carousel">
          <button
            className="tradeclients-arrow tradeclients-arrow-left"
          ...
          <div className="tradeclients-scroller">
            <ul className="tradeclients-track" ref={trackRef}>
              {logosDup.map((src, i) => (
                <li className="tradeclients-card" key={i}>
                  <figure className="tradeclients-logo">
                    <img
                      src={src}
                      alt={`client-${i}`}
                      className="tradeclients-img"
                    />
                  </figure>
                </li>
              ))}
          ...
          <button
            className="tradeclients-arrow tradeclients-arrow-right"
