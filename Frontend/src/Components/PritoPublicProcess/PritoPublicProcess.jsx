...
const GSTProcess = () => {
  return (
    <section className="prp-process-wrapper">
      <h2 className="prp-process-heading">
        STEPS FOR PRIVATE LIMITED COMPANY REGISTRATION IN INDIA
      </h2>
      <p className="prp-process-subheading">
        The broad process of registering a Private Limited company involves
        the following steps:
      </p>

      <div className="prp-process-timeline">
        <div className="prp-process-line" />

        {steps.map((step, index) => (
          <div
            key={index}
            className={`prp-process-item ${
              index % 2 === 0 ? "prp-process-left" : "prp-process-right"
            }`}
          >
            <div className="prp-process-dot">{index + 1}</div>

            <div className="prp-process-card">
              <h4>{step.title}</h4>
              <p>{step.text}</p>
            </div>
          </div>
