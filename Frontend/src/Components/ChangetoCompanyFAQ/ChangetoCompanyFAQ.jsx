...
const FaqPvt = () => {
  ...
  return (
    <section className="prp-faq-section">
      <div className="prp-faq-container">

        {/* Left side */}
        <div className="prp-faq-left">
          <h2 className="prp-faq-title">
            Private Limited Company Registration FAQ&apos;s
          </h2>

          <p className="prp-faq-intro">
          ...
          <div className="prp-faq-list">
            {faqs.map((item, index) => {
              ...
              return (
                <div
                  key={index}
                  className={`prp-faq-item ${isActive ? "active" : ""}`}
                >
                  <button
                    type="button"
                    className="prp-faq-question"
                    onClick={() => toggleFaq(index)}
                  >
                    <span>{item.question}</span>
                    <span
                      className={`prp-faq-icon ${isActive ? "open" : ""}`}
                    >
                      ▾
                    </span>
                  </button>
                  ...
                  {isActive && (
                    <div
                      className={`prp-faq-answer ${isActive ? "open" : ""}`}
                    >
                      <p>{item.answer}</p>
                    </div>
                  )}
