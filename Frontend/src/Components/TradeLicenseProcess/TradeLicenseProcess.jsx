...
import "./TradeLicenseProcess.css";
...
const TradeLicenseProcess = () => {
  ...
  const handleScroll = (direction) => {
    ...
    const card = container.querySelector(".tradeproc-card");
    if (!card) return;
  ...
  return (
    <section className="tradeproc-section">
      <div className="tradeproc-container">
        <h2 className="tradeproc-heading">
          See What Our Customers Say on Google
        </h2>

        <div className="tradeproc-slider-wrapper">
          {/* Left arrow */}
          <button
            className="tradeproc-side-arrow tradeproc-left"
          ...
          {/* Cards slider */}
          <div className="tradeproc-slider" ref={sliderRef}>
            {testimonials.map((t, idx) => (
              <article className="tradeproc-card" key={idx}>
                <span className="tradeproc-quote tradeproc-quote-top">“</span>

                <div className="tradeproc-avatar-wrap">
                  <div className="tradeproc-avatar">
                    <span>{t.initial}</span>
                  </div>
                </div>

                <p className="tradeproc-text">{t.text}</p>

                <h3 className="tradeproc-name">{t.name}</h3>
                <p className="tradeproc-role">{t.role}</p>

                <div className="tradeproc-stars">
                  {Array.from({ length: t.rating }).map((_, i) => (
                    <span key={i}>★</span>
                  ))}
                </div>

                <span className="tradeproc-quote tradeproc-quote-bottom">”</span>
              </article>
            ))}
          </div>
          ...
          {/* Right arrow */}
          <button
            className="tradeproc-side-arrow tradeproc-right"
