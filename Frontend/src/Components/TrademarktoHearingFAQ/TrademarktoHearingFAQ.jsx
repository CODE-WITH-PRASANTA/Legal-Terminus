...
import "./TrademarktoHearingFAQ.css";
...
const TradeLicenseFAQ = () => {
  ...
  return (
    <section className="thfaq-section">
      <div className="thfaq-container">
        {/* Left side */}
        <div className="thfaq-left">
          <h2 className="thfaq-title">Trade License Registration FAQ&apos;s</h2>

          <p className="thfaq-intro">
        ...
        {/* Right side */}
        <div className="thfaq-right">
          <div className="thfaq-list">
            {faqs.map((item, index) => {
              const isActive = index === activeIndex;
              return (
                <div
                  key={index}
                  className={`thfaq-item ${isActive ? "active" : ""}`}
                >
                  <button
                    type="button"
                    className="thfaq-question"
                    onClick={() => toggleFaq(index)}
                  >
                    <span>{item.question}</span>
                    <span className={`thfaq-icon ${isActive ? "open" : ""}`}>
                      ▾
                    </span>
                  </button>

                  <div
                    className={`thfaq-answer ${isActive ? "open" : ""}`}
                  >
                    <p>{item.answer}</p>
                  </div>
                </div>
