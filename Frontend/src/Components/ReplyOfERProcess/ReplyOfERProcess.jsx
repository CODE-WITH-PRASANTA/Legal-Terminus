...
const GSTProcess = () => {
  return (
    <section className="Replyof-ER-gst-wrapper">
      <h2 className="Rf-ER-gst-heading">
        STEPS FOR PRIVATE LIMITED COMPANY REGISTRATION IN INDIA
      </h2>
      <p className="Rf-ER-gst-subheading">
        The broad process of registering a Private Limited company involves
        the following steps:
      </p>

      <div className="Replyof-ER-timeline">
        <div className="Replyof-ER-timeline-line" />

        {steps.map((step, index) => (
          <div
            key={index}
            className={`Replyof-ER-timeline-item ${
              index % 2 === 0 ? "left" : "right"
            }`}
          >
            <div className="Replyof-ER-timeline-dot">{index + 1}</div>

            <div className="Replyof-ER-timeline-card">
              <h4>{step.title}</h4>
              <p>{step.text}</p>
            </div>
          </div>
